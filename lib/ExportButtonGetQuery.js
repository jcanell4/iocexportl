require([
        ...
        if(exportPdfButton){
            ...
            exportPdfButton.onClick = fOnClick;            //on(exportPdfButton, "click", fOnClick); S'aha canviat la sentencia perquè amb la versió 1.10.4 de dojo ha deixat de funcionar!
        }
        if(exportHtmlButton){
            exportHtmlButton.getQuery=fGetQuery;       
            exportHtmlButton.set("hasTimer", true);
            exportHtmlButton.onClick = fOnClick;            //on(exportHtmlButton, "click", fOnClick);
        }
        if(exportOnePdfButton){
            exportOnePdfButton.getQuery=fGetQuery;
            exportOnePdfButton.set("hasTimer", true);
            exportOnePdfButton.onClick = fOnClick;            //on(exportOnePdfButton, "click", fOnClick);
        }
});
